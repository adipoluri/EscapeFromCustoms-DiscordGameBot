...
module.exports = {
  ...
  execute(message) {
      ...
      con.query("Select * FROM PlayersInRaid", function (err, result) {
        ...
        setTimeout(function () {
          if (
            ...
            con.query(
              "INSERT INTO `PlayersInRaid` (UserID) VALUES (" + ID + ")"
            );
            con.query(
              "UPDATE `PlayersInRaid`" +
                "set STR = " +
                Stats[0] +
                " Where USERID = " +
                ID
            ...
            if (!raidstarted) {
              message.channel.send(
                "You are already queued or have not registered, make sure to user !register"
              );
            } else {
    ...
    myEmitter.on("alreadyinraid", () => {
      console.log(
        "User who used !raid is, first to initialize a new raid: ",
        first,
        "already in a raid: ",
        alreadyinraid,
        "already registered: ",
      ...
      if (
        ...
        console.log(
          "User has met all requirements, adding them to playersinraid database!"
        );
          ...
          if (time == 0) {
            ...
            con.query("Select * from PlayersInRaid", function (err, result) {
              ...
              for (var i = 0; i < result.length; ) {
                ...
                message.channel.send(
                  rollencounter(num, players[i], encounterType)
                );
                i++;
              }
            });
    ...
    var rollencounter = function (rng, player, encounterType, i) {
      const roll = new Discord.MessageEmbed()
        ...
        .setThumbnail(
          "https://cdnb.artstation.com/p/assets/images/images/018/042/671/large/hayo-sena-00.jpg?1558240182"
        );
